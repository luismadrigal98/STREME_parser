...
STREME Analysis Pipeline - MaPipeline Steps:
  ...
  5. full             - Run complete pipeline (steps 1-3)CLI tool for motif di    # Feature extraction subcommand
    ...
    # Mixed-effects analysis subcommand (RECOMMENDED)
    mixed_parser = subparsers.add_parser('analyze-mixed', help='Mixed-effects analysis (RECOMMENDED approach)')
    mixed_parser.add_argument('motif_features', help='Motif feature file from extract-features')
    mixed_parser.add_argument('expression_data', help='Expression data file (Gene, Line, Expression)')
    mixed_parser.add_argument('--approach', choices=['mixed', 'gene-by-gene', 'hierarchical', 'all'],
                             default='all', help='Statistical approach')
    mixed_parser.add_argument('--output', '-o', default='mixed_effects_results', help='Output directory')
...
Examples:
  ...
  %(prog)s full /path/to/streme/results /path/to/sequences/ --lines IM502,IM664,IM767
    ...
    if args.command == 'consolidate':
        ...
        cmd = [
            'python', str(project_root / 'cli_tools' / 'mixed_effects_analyzer.py'),
            args.motif_features,
            args.expression_data,
        ...
        success = run_command(cmd, "Running mixed-effects analysis")
