#!/usr/bin/env python3
"""
Motif Catalog Builder - CLI tool for consolidating STREME motifs across multiple lines
"""
...
def process_all_lines_sequential(base_dir, similarity_threshold=0.75, verbose=False):
    """
    ...
    for line_idx, line_dir in enumerate(line_dirs):
        ...
        for motif in line_motifs:
            ...
            for i, existing_motif in enumerate(unique_motifs):
                if sequences_are_similar(motif['consensus'], existing_motif['consensus'], similarity_threshold):
...
def write_consolidated_output(unique_motifs, motif_line_occurrence, line_motif_map, output_prefix="motif_catalog"):
    """Write all output files"""
    
    # 1. Consolidated catalog
    catalog_file = f"{output_prefix}.txt"
    ...
    maps_file = f"{output_prefix}_line_maps.txt"
    ...
    # 3. ML strings
    ml_file = f"{output_prefix}_ml_strings.fasta"
    ...
    return catalog_file, maps_file, ml_file

def main():
    parser = argparse.ArgumentParser(
        description="Build consolidated motif catalog from STREME outputs across multiple lines",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage
  %(prog)s /path/to/streme/results/
  
  # With custom output prefix and similarity threshold
  %(prog)s /path/to/streme/results/ --output my_motifs --threshold 0.8 --verbose
        """
    )
    
    parser.add_argument(
        'input_dir',
        help='Directory containing STREME output directories (streme_Genes_*)'
    )
    
    parser.add_argument(
        '--output', '-o',
        default='motif_catalog',
        help='Output file prefix (default: motif_catalog)'
    )
    ...
    # Write outputs
    catalog_file, maps_file, ml_file = write_consolidated_output(
