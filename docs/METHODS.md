...
When matched gene-expression data are unavailable, the consolidated motif sites are explored with two complementary unsupervised analyses (`network` subcommand). First, a motif co-occurrence network is constructed: for every pair of motifs `(i, j)` passing a minimum per-gene frequency filter, the number of promoters carrying both motifs is compared against the marginal counts using a one-sided Fisher's exact test for enrichment, with Benjamini–Hochberg q-value control. Pairs passing a Jaccard or lift threshold and the FDR cutoff are retained as network edges. Connected components of the thresholded graph are reported as motif modules — sets of motifs that systematically co-occur in promoters and represent candidate combinatorial regulatory programs. The network is exported in GraphML format for downstream visualisation in Cytoscape or Gephi. Second, genes are clustered by their motif-presence profiles using hierarchical clustering on the Jaccard distance with average linkage; each cluster is characterised by a motif "fingerprint", computed as the motifs significantly enriched within the cluster relative to the genome background under a one-sided Fisher's exact test with BH-corrected q-values. A complementary per-motif positional summary (distance from promoter start and relative position within the promoter) is reported to surface motifs with proximal or distal preferences. When TOMTOM annotations are available, the motif IDs in modules and fingerprints can be cross-referenced to the corresponding transcription factors, recasting the analyses as TF cooperativity networks and TF cluster signatures.
