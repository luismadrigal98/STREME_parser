...
The consolidated TSV file contains the following 17 columns, providing comprehensive information about motif occurrences across genomes:

> **Terminology:** the canonical term is **genome**. The `genome` column was previously named `line`; the legacy `line` name is still accepted as an alias on input, so existing Mimulus data needs no migration.

### 1. `consolidated_motif_id`
**Format:** `MOTIF_001`, `MOTIF_002`, etc.
**Description:** A unique identifier assigned to each consolidated motif cluster. Motifs with similar consensus patterns (ignoring numeric prefixes from STREME) are grouped together and assigned the same consolidated ID. This allows tracking of the same regulatory element across different genomes.
...
### 5. `genome`
**Format:** `P_virgatus`, `P_barbatus`, `IM767`, etc.
**Description:** The genome identifier, extracted from the directory name containing the STREME analysis (e.g. `streme_P_virgatus/`). Represents the different genomes being analyzed — distinct species/assemblies of a genus, or ecotypes/inbred lines of one species. The legacy column name `line` is accepted as an alias on input.

### 6. `gene_id`
**Format:** `Peviz.001G000100`, `Migut.A00001.v2.0`, etc.
**Description:** The unique identifier for the gene containing this motif occurrence. Corresponds to gene annotations in the genome assembly being analyzed.
...
**Description:** The sequential position of this motif within the gene (per genome), ordered by genomic position. The first motif in a gene has `relative_position = 1`, the second has `relative_position = 2`, etc. Useful for analyzing regulatory element organization.

### 13. `total_motifs_in_gene`
**Format:** Integer (e.g., `3`, `7`, `12`)
**Description:** The total number of motif occurrences found within this gene in this specific genome. Indicates the regulatory complexity or density for this gene.
...
**Description:** The number of original STREME motif variants that were consolidated into this `consolidated_motif_id`. A higher number indicates this motif pattern was found with more sequence variations across different genomes or within the same analysis.
...
```bash
# Genes with more than 5 motifs in any genome (col 13 = total_motifs_in_gene)
awk -F'\t' '$13 > 5' consolidated_motifs.tsv | cut -f5,6 | sort | uniq
```
...
### Comparing regulatory complexity across genomes:
```bash
# Average motifs per gene by genome (col 5 = genome, col 13 = total_motifs_in_gene)
awk -F'\t' 'NR>1 {sum[$5] += $13; count[$5]++} END {for(g in sum) print g, sum[g]/count[g]}' consolidated_motifs.tsv
```

### Identifying heavily merged regions (poly-nucleotide tracts):
```bash
# Find motifs that were merged from many overlapping hits (col 16 = merged_count)
awk -F'\t' '$16 > 3' consolidated_motifs.tsv | cut -f1,2,5,6,7,16 | head -10
```

### Quality control for overlap merging:
```bash
# Check distribution of merged_count values (col 16)
cut -f16 consolidated_motifs.tsv | sort -n | uniq -c
...
- **Sorting:** Output is sorted by consolidated_motif_id, then genome, then gene_id, then start_pos for easy analysis
...
- Original STREME `sites.tsv` files in respective `streme_<genome>/` directories
